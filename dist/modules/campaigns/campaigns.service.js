...
let CampaignsService = CampaignsService_1 = class CampaignsService {
    ...
    async createCampaign(userId, dto) {
        ...
        if (saved.hashtags?.length || saved.mentions?.length) {
            setTimeout(() => this.processCampaign(saved.id).catch(err => this.logger.error(`Campaign processing failed for ${saved.id}: ${err.message}`)), 2000);
        }
        return saved;
    }
    ...
    async addInfluencer(userId, campaignId, dto) {
        ...
        setTimeout(() => this.enrichInfluencer(saved, campaign.platform, userId).catch(err => this.logger.warn(`Influencer enrichment failed for ${saved.id}: ${err.message}`)), 500);
        return saved;
    }
    async enrichInfluencer(influencer, platform, userId) {
        ...
        try {
            ...
            if (insight && Number(insight.followerCount) > 0) {
                ...
                influencer.audienceCredibility = insight.audienceCredibility
                    ? Number(insight.audienceCredibility) : undefined;
                enriched = true;
            }
        }
        catch { }
        if (!enriched && this.modashService.isModashEnabled()) {
            try {
                const platformType = (platform?.toUpperCase() || 'INSTAGRAM');
    ...
    async addPost(userId, campaignId, dto) {
        ...
        if (!linkedInfluencerId && parsed.username) {
            ...
            if (matchingInf)
                linkedInfluencerId = matchingInf.id;
        }
        ...
        if (searchTerms.length > 0) {
            ...
            setTimeout(() => this.enrichPost(saved, campaign.platform, userId).catch(err => this.logger.warn(`Post enrichment failed for ${saved.id}: ${err.message}`)), 500);
        }
        return { post: saved, warning };
    }
    async enrichAndValidatePost(post, campaign, userId) {
        try {
    ...
    async enrichPost(post, campaignPlatform, userId) {
        if (!post.postUrl || !this.modashService.isModashEnabled())
            return;
        ...
        try {
            const platformType = (campaignPlatform?.toUpperCase() || 'INSTAGRAM');
            const report = await this.modashService.getInfluencerReport(platformType, username, userId);
            if (!report)
                return;
            const rpt = report;
    ...
    async processCampaign(campaignId) {
        ...
        try {
            ...
            const platformType = (campaign.platform?.toUpperCase() || 'INSTAGRAM');
            const searchTerms = [...(campaign.hashtags || []), ...(campaign.mentions || [])];
            ...
            const existingPostUrls = new Set((await this.postRepo.find({ where: { campaignId }, select: ['postUrl'] })).map(p => p.postUrl));
            for (const inf of existingInfluencers) {
                ...
                if (report) {
                    const profile = report.profile || report;
                    allPosts.push(...(profile.recentPosts || report.recentPosts || []), ...(profile.topPosts || report.topPosts || []), ...(profile.popularPosts || report.popularPosts || []));
                }
                ...
                for (const post of allPosts) {
                    ...
                    const hashtags = (post.hashtags || []).map((h) => h.toLowerCase().replace(/^#/, ''));
                    const mentions = (post.mentions || []).map((m) => m.toLowerCase().replace(/^@/, ''));
                    const matchesSearch = searchKeywords.length === 0 || searchKeywords.some(kw => text.includes(kw) || hashtags.includes(kw) || mentions.includes(kw));
                ...
                inf.status = campaign_entity_1.InfluencerStatus.ACTIVE;
                ...
                if (fc > 0 && inf.postsCount > 0) {
                    ...
                    await this.postRepo
                        .createQueryBuilder()
                        .update(campaign_entity_1.CampaignPost)
    ...
    computeCampaignStatus(campaign) {
        if (campaign.status === campaign_entity_1.CampaignStatus.CANCELLED || campaign.status === campaign_entity_1.CampaignStatus.PAUSED) {
        ...
        if (end && now > end)
            return campaign_entity_1.CampaignStatus.COMPLETED;
        if (start && now < start)
            return campaign_entity_1.CampaignStatus.PENDING;
        return campaign_entity_1.CampaignStatus.ACTIVE;
    }
            ...
            try {
                ...
                if (Array.isArray(ages)) {
                    for (const a of ages) {
                        ...
                        if (!ageAgg.has(range))
                            ageAgg.set(range, { male: 0, female: 0, total: 0 });
...
exports.CampaignsService = CampaignsService = CampaignsService_1 = __decorate([
    ...
    __param(7, (0, typeorm_1.InjectRepository)(influencer_insight_entity_1.InfluencerInsight)),
